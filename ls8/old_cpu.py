...
class CPU:
    ...
    def __init__(self):
        ...
        """Construct a new CPU."""
        """ R7 is reserved as the stack pointer (SP). These registers only hold values between 0-255. """
        self.ram = [0] * 256
        self.reg = [0] * 8
        self.pc = 0
        self.SP = 7
    ...
    def run(self):
        ...
        while running:
            ...
            # now we have to go through 8 instructions seen above.
            
            # if the Instruction Register is Halted, this isn't running 
            if IR == HLT:
                running = False
            # elsif the integer is the same as (will use IR from here on) the IR
            elif IR == LDI:
